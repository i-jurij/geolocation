...
function htmlReplace() {
  showHideModal("city_region_info_div", "hide");

  let inner_message_footer =
...
function htmlInfo({ city, region }) {
  let info = "";
  if (city && typeof city == "string") {
    info =
      "Ваше местоположение: " +
      regionWithoutCity(city, region) +
      ' Если нет - выберите его, нажав на кнопку "Выбрать"';
    newData("location", city);
    newData("clients_location_message", info);
    // showHideModal("city_region_info_div", 'show');
    setTimeout(() => {
      showHideModal("show_city_select", "hide");
    }, 200);
  } else {
    info =
      'Ваше местоположение неизвестно. </br>Выберите его, нажав на кнопку "Выбрать"';
    newData("clients_location_message", info);
    showHideModal("city_region_info_div", "hide");
  }
}
...
class Locality {
  ...
  async init() {
    ...
    if (this.l.city) {
      if (
        city_from_back &&
        !city_from_back.toLowerCase().includes(this.l.city.toLowerCase())
      ) {
        this.localityToServer(
          url_location_to_server_js,
          csrf,
          this.l.city,
          this.l.region
        );
      ...
      if (city_from_back == unknown_location) {
        ...
        this.LS.setLocalityToLocalStorage({
          city: city_from_back,
          region: region_from_back,
        });
      }
    }
  }
  ...
  fromDB() {
    ...
    if (save_city) {
      save_city.addEventListener("pointerdown", () => {
        let region_from_select =
          shoose_region.options[shoose_region.selectedIndex].text;
        let city_from_select =
          shoose_city.options[shoose_city.selectedIndex].text;
        if (
          city_from_select &&
          region_from_select &&
          city_from_select != "Город"
        ) {
          this.localitySaveAndShow(
            url_location_to_server_js,
            csrf,
  ...
  async getAllLocations() {
    ...
    if (
      all_locality === null ||
      all_locality.length < 1 ||
      all_locality === "undefined"
    ) {
      ...
      resultsList: {
        element: (list, data) => {
          if (!data.results.length) {
            ...
            message.innerHTML =
              "<span>Не найдено " + sanitize(data.query) + "</span>";
