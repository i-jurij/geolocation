...
по умолчанию на текущую страницу.
...
**_PHP часть не зависит от javascript. Javascript добавляет возможность получения местоположения пользователя по координатам, если по IP определить не вышло, а также изменить содержимое элементов страницы без ее полной перезагрузки._**
...
**_If you install this library by npm this dependencies will be installed too._**  
...
function _construct(){
    ...
    /// not necessary ///
    $this->geo->ip_provider = 'sypexgeo'; // geoplugin (only pay), sypexgeo, ipapicom, ipapico, ipwho
...
Library set js fetch request headers "'Accept': 'application/json'" and set credentials to "'same-origin'".
